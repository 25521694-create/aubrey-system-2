...
export async function generateStaticParams() {
  return getAllResearchSlugs("general-research").map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params
  const article = getResearchArticleBySlug("general-research", slug)
  if (!article) return {}
  return {
...
export default async function GeneralResearchArticlePage({ params }: PageProps) {
  const { slug } = await params
  const article = getResearchArticleBySlug("general-research", slug)
  if (!article) notFound()
